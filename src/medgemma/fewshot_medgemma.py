...
OUTPUT_DIR = f"medgemma_few_{SET_ID}"
RESULTS_CSV = os.path.join(OUTPUT_DIR, f"results_medgemma_set{SET_ID}.csv")
...
def _clean_entities(
    ...
    for ent in entities:
        ...
        key = (start_i, end_i, str(label).strip())
        if key in seen:
            continue
        seen.add(key)
    ...
    cleaned.sort(key=lambda x: (x["start"], x["end"], x["label"], x["text"]))
    return cleaned
...
def _fewshot_response_json(example: Dict[str, Any]) -> str:
    payload = {
        "entities": [
            {"label": label, "text": surface}
            for label, surface in example["entities"]
        ]
    }
...
def system_prompt(labels: Sequence[str]) -> str:
    ...
    parts = [instructions, "", "Here are labeled examples:"]
    for idx, example in enumerate(FEWSHOT_EXAMPLES, start=1):
        parts.extend(
            [
                ...
                _fewshot_response_json(example),
                "",
            ]
        )
    parts.append("Follow the same format for the next passage.")
    return "\n".join(parts)
...
def build_messages(example: Example, labels: Sequence[str]) -> List[Dict[str, str]]:
    return [
...
def compute_dynamic_max_new_tokens(
    ...
    gold_outputs = [gold_response_json(ex) for ex in train_examples]
    if not gold_outputs:
        raise ValueError("Cannot compute max_new_tokens from an empty training dataset.")

    lengths = [len(tokenizer(output, add_special_tokens=False)["input_ids"]) for output in gold_outputs]
    longest = max(lengths)
    doubled = max(1, 2 * longest)
    clipped = min(doubled, cap)

    LOGGER.info(
        "Training gold output token lengths: min=%d median=%d max=%d | dynamic max_new_tokens=%d | cap=%d",
        min(lengths),
    ...
    if cap < longest:
        LOGGER.warning(
            "model_max_output_tokens=%d is smaller than the longest gold output (%d tokens). "
            "Some evaluation generations may be truncated; consider increasing the cap above %d.",
            cap,
            longest,
            longest,
        )
...
def generate_prediction(
    ...
    messages = build_messages(example, labels)
...
def prediction_path(output_dir: str, stage: str, save_predictions: bool) -> Optional[str]:
    return os.path.join(output_dir, f"{stage}_predictions.jsonl") if save_predictions else None
...
def main() -> None:
    ...
    LOGGER.info("Loading datasets from JSON/JSONL")
    ...
    if cfg.max_new_tokens is None:
        cfg.max_new_tokens = compute_dynamic_max_new_tokens(
            tokenizer=tokenizer,
    ...
    fewshot_pred_path = prediction_path(cfg.output_dir, "fewshot", cfg.save_predictions)

    LOGGER.info("Running few-shot evaluation on test set")
    all_rows = evaluate_model(
        ...
        save_predictions_path=fewshot_pred_path,
    )
